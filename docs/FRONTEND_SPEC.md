...
- **Limitación conocida (CR6):** `GET /crm/agenda` es `client_admin`+, pero la pantalla de la puerta es del `staff`. Un `staff` puede escanear y hacer check-in (esos endpoints aceptan cualquier rol), pero **no puede elegir el evento** ni, por lo tanto, abrir la lista de asistencia ni provocar el rechazo por `wrong_event`. Necesita una decisión del backend: abrir la agenda a lectura para `staff`, o exponer el evento activo por una ruta propia.
