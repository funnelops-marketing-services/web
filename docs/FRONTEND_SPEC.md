...
- **Conciliación de pagos (CR4, web#184 / server#274):** `GET /crm/payments/pending` → `{ total, items }` con solo los pagos que **el sistema** aprobó y nadie cotejó (los validados a mano no entran) · `POST /crm/payments/{id}/confirm` (sin body, idempotente) · `POST /crm/payments/{id}/reject` con `{ note }` obligatoria (≥3 caracteres tras normalizar espacios; 422 si no, 400 del dominio) → **revoca la entrada** y mueve la card al stage `lost` · `GET /crm/payments/export?day=YYYY-MM-DD` → CSV (`day` opcional, default hoy **en UTC**). Pantalla propia en `/pagos`, visible para los 3 roles, con el contador en el menú (`components/crm/payments/`). `amount`, `currency` y `paid_at` (date-only) se muestran **sin reformatear**, mismo criterio que el panel del comprobante.
